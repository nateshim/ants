...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if insect.is_ant:
            # Phase 4: Special Handling for BodyguardAnt ind QueenAnt
            if hasattr(insect, 'real_queen') and insect.real_queen:
                return 
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while curr_place != hive:
            if curr_place.bees: 
                if self.min_range <= place_position and place_position <= self.max_range:# and place_position != 4:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        # BEGIN Problem 4A
        "*** REPLACE THIS LINE ***"
        #if the fire ants armor == 0, reduce the bees armor by amount
        self.armor -= amount
        if self.armor <= 0:
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        ...
        if self.can_contain(ant):    
            self.ant = ant
            #self.place.remove_insect(self.ant)
        # END Problem 7
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 9
        "*** REPLACE THIS LINE ***"
        curr_location = self.place.exit
        if self.real_queen == False:
            self.reduce_armor(self.armor)
            return
        self.throw_at(self.nearest_bee(colony.hive))
        while curr_location:
            #print(type(curr_location.ant))
            if curr_location.ant:
                if not curr_location.ant.been_doubled:
                    curr_location.ant.damage = curr_location.ant.damage*2
                    curr_location.ant.been_doubled = True
                if hasattr(curr_location.ant, "ant") and curr_location.ant.ant and not curr_location.ant.ant.been_doubled:
                    curr_location.ant.ant.damage = curr_location.ant.ant.damage*2 
                    curr_location.ant.ant.been_doubled = True
            curr_location = curr_location.exit
        # END Problem 9
